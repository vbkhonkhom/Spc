    ...
    Public Sub GraphDisp7(ByVal Size As String)
        ...
        Try
            ...
            If Size = "MAX" Then
                ...
                yh = 428
            ElseIf Size = "Middle" Then
                pb = FormMiddle.PictureBox9
                yh = 362
            ElseIf Size = "MIN" Then
                pb = FormSmall.PictureBox9
                yh = 260
            Else
                pb = Form1.PictureBox9
                yh = 428
            ...
            Using APen As New Pen(Color.Green, 2) With {.DashStyle = Drawing2D.DashStyle.Dot},
                ...
                If X_Shiguma = 0 And HistogramCount > 1 Then
                    ...
                    For i = 0 To HistogramCount - 1
                        Dim dblval As Double = Val(HistogramBuf(i))
                        h_sum += dblval
                        h_sumSq += dblval * dblval
                    Next
                ...
                Dim n_Cl_s1 As Integer = 0
                Dim n_s1_s2 As Integer = 0, n_s2_s3 As Integer = 0, n_s3_s4 As Integer = 0, n_s4_s5 As Integer = 0
                Dim n_s5_s6 As Integer = 0, n_s6_s7 As Integer = 0, n_s7_s8 As Integer = 0, n_s8_s9 As Integer = 0, n_s9_s10 As Integer = 0

                Dim n_Cl_ms1 As Integer = 0
                Dim n_ms1_ms2 As Integer = 0, n_ms2_ms3 As Integer = 0, n_ms3_ms4 As Integer = 0, n_ms4_ms5 As Integer = 0
                Dim n_ms5_ms6 As Integer = 0, n_ms6_ms7 As Integer = 0, n_ms7_ms8 As Integer = 0, n_ms8_ms9 As Integer = 0, n_ms9_ms10 As Integer = 0
                    ...
                    For i = 0 To HistogramCount - 1
                        ...
                        ElseIf dblData > (X_CL - (X_Shiguma / 2)) Then
                            If dblData <= (X_CL + X_Shiguma * 1.5) Then
                                n_s1_s2 += 1
                            ElseIf dblData <= (X_CL + X_Shiguma * 2.5) Then
                                n_s2_s3 += 1
                            ElseIf dblData <= (X_CL + X_Shiguma * 3.5) Then
                                n_s3_s4 += 1
                            ElseIf dblData <= (X_CL + X_Shiguma * 4.5) Then
                                n_s4_s5 += 1
                            ElseIf dblData <= (X_CL + X_Shiguma * 5.5) Then
                                n_s5_s6 += 1
                            ElseIf dblData <= (X_CL + X_Shiguma * 6.5) Then
                                n_s6_s7 += 1
                            ElseIf dblData <= (X_CL + X_Shiguma * 7.5) Then
                                n_s7_s8 += 1
                            ElseIf dblData <= (X_CL + X_Shiguma * 8.5) Then
                                n_s8_s9 += 1
                            Else
                                n_s9_s10 += 1
                            End If
                        ElseIf dblData <= (X_CL - (X_Shiguma / 2)) Then
                            ...
                            ElseIf dblData > (X_CL - X_Shiguma * 8.5) Then
                                n_ms7_ms8 += 1
                            ElseIf dblData > (X_CL - X_Shiguma * 9.5) Then
                                n_ms8_ms9 += 1
                            Else
                                n_ms9_ms10 += 1
                            End If
                ...
                ElseIf Size = "Middle" Then
                    lbl24 = FormMiddle.Label24 : lbl25 = FormMiddle.Label25 : lbl26 = FormMiddle.Label26 : lbl27 = FormMiddle.Label27 : lbl28 = FormMiddle.Label28
                Else
                ...
                lbl27.Text = Format(ScaleUnit * 3).ToString()
                lbl28.Text = Format(ScaleUnit * 4).ToString()

                Cl_s1 = CInt(n_Cl_s1 * dynamicBarWidth)
                s1_s2 = CInt(n_s1_s2 * dynamicBarWidth) : s2_s3 = CInt(n_s2_s3 * dynamicBarWidth) : s3_s4 = CInt(n_s3_s4 * dynamicBarWidth)
                s4_s5 = CInt(n_s4_s5 * dynamicBarWidth) : s5_s6 = CInt(n_s5_s6 * dynamicBarWidth) : s6_s7 = CInt(n_s6_s7 * dynamicBarWidth)
                s7_s8 = CInt(n_s7_s8 * dynamicBarWidth) : s8_s9 = CInt(n_s8_s9 * dynamicBarWidth) : s9_s10 = CInt(n_s9_s10 * dynamicBarWidth)

                Cl_ms1 = CInt(n_Cl_ms1 * dynamicBarWidth)
                ms1_ms2 = CInt(n_ms1_ms2 * dynamicBarWidth) : ms2_ms3 = CInt(n_ms2_ms3 * dynamicBarWidth) : ms3_ms4 = CInt(n_ms3_ms4 * dynamicBarWidth)
                ms4_ms5 = CInt(n_ms4_ms5 * dynamicBarWidth) : ms5_ms6 = CInt(n_ms5_ms6 * dynamicBarWidth) : ms6_ms7 = CInt(n_ms6_ms7 * dynamicBarWidth)
                ms7_ms8 = CInt(n_ms7_ms8 * dynamicBarWidth) : ms8_ms9 = CInt(n_ms8_ms9 * dynamicBarWidth) : ms9_ms10 = CInt(n_ms9_ms10 * dynamicBarWidth)
...
#Region "?????"
    ...
    Private Sub PrintPageHandler(ByVal sender As Object, ByVal e As PrintPageEventArgs)
        ...
        Dim histoWidth As Integer = CInt(m.Width * 0.145)
        ...
        Dim slotWidth As Double = xBarWidth / 30.0
        Dim lastPoint As Point = Point.Empty
        For j As Integer = 0 To 29
            Dim k = DispStartPosition + j
            If k < SPCDataNum Then
                Dim valX = Val(readMaster(M_Data(k), _X))
                Dim px = CInt(dRectX.Left + (j * slotWidth) + (slotWidth / 2))
                Dim py = getYFinal(valX)

                If Not lastPoint.IsEmpty Then g.DrawLine(blueGridPen, lastPoint, New Point(px, py))

                Dim dotColor = If(valX > uclVal Or valX < lclVal, Color.Red, Color.Blue)
                g.FillEllipse(New SolidBrush(dotColor), px - 3, py - 3, 6, 6)
                g.DrawEllipse(Pens.Black, px - 3, py - 3, 6, 6)
                lastPoint = New Point(px, py)
            End If
        Next

        If HistoCounts.Count > 0 Then
            Dim maxCount = 1
            For Each c In HistoCounts
                If c > maxCount Then
                    maxCount = c
                End If
            Next
        ...
        Dim dRectR As New Rectangle(m.Left + scaleSpace, curY, xBarWidth, commonGraphHeight)

        Dim valRTop As Double = Val(Form1.LabR(0).Text)
        Dim valRBot As Double = Val(Form1.LabR(9).Text)
        Dim vRRange As Double = valRTop - valRBot

        For i As Integer = 0 To 9
            Dim ly = dRectR.Top + CInt(i * (commonGraphHeight / 9))
            g.DrawLine(blackGridPen, dRectR.Left, ly, dRectR.Right, ly)
            g.DrawString(Form1.LabR(i).Text, labelFont, Brushes.Black, m.Left + 5, ly - 5)
        Next
        Dim rSlotW As Double = xBarWidth / 30.0
        For j As Integer = 0 To 30
            Dim lx = CInt(dRectR.Left + (j * rSlotW))
            g.DrawLine(blackGridPen, lx, dRectR.Top, lx, dRectR.Bottom)
        Next
        ...
        If vRRange > 0 Then
            ...
            For j As Integer = 0 To 29
                Dim k = DispStartPosition + j
                If k < SPCDataNum Then
                    Dim valR As Double = Val(readMaster(M_Data(k), _R))
                    Dim px = CInt(dRectR.Left + (j * rSlotW) + (rSlotW / 2))
                    Dim py = getRY(valR)

                    If Not lastRPoint.IsEmpty Then g.DrawLine(blueGridPen, lastRPoint, New Point(px, py))

                    Dim dotBrush = If(valR > ruclVal, Brushes.Red, Brushes.Blue)
                    g.FillEllipse(dotBrush, px - 3, py - 3, 6, 6)
                    g.DrawEllipse(Pens.Black, px - 3, py - 3, 6, 6)
                    lastRPoint = New Point(px, py)
                End If
            Next
        End If
        Using waterMarkR As New Font("Arial", 80, FontStyle.Bold)
            Using waterMarkBrushR As New SolidBrush(Color.FromArgb(40, Color.Gray))
                Dim waterMarkTextR As String = "R"
                Dim textSize As SizeF = g.MeasureString(waterMarkTextR, waterMarkR)

                Dim posX As Single = dRectR.Left + (dRectR.Width - textSize.Width) / 2
                Dim posY As Single = dRectR.Top + (dRectR.Height - textSize.Height) / 2

                g.DrawString(waterMarkTextR, waterMarkR, waterMarkBrushR, posX, posY)
            End Using
        End Using
